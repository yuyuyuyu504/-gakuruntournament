...
constructor() {
  ...
  loadMatches() {
    const raw = localStorage.getItem('gaku_tournament_matches');
    if (raw !== null) {
      try {
        const parsed = JSON.parse(raw);
...
(function() {
  ...
  document.addEventListener("DOMContentLoaded", function() {
    ...
    // Always set up navigation regardless of app status
    window.addEventListener("hashchange", handleRoute);

    // Direct click binding on nav links
    document.querySelectorAll(".nav-link, .logo-link").forEach(function(link) {
